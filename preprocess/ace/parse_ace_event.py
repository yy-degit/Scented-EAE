...
def one_fold(fold, doc_path, output_dir, split_path, heads_only=True, real_entities_only=True, include_pronouns=False):
    doc_keys = []
    ...
    with open(path.join(output_dir, fold + ".json"), "w") as g:
        ...
        for doc_key in doc_keys:
            ...
            js = document.to_json()
            res.append(js)
        json.dump(res, g, indent=4, ensure_ascii=False, default=int)


def main():
    parser = argparse.ArgumentParser(description="Preprocess ACE event data.")
    parser.add_argument("--data_dir", default="data/dataset/ace/target" , help="Name for data directory.")
    parser.add_argument("--output_dir", default="data/row_data/ace" , help="Name for output directory.")
    parser.add_argument("--split_dir", default="data/dataset/ace/event_split" , help="Name for split configuration directory.")

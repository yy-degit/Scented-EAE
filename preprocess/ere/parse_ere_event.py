...
def process_batch(input_dir, output_file, dataset='normal', language='english'):
    ...
    if dataset == 'normal':
        source_files = glob.glob(
            os.path.join(input_dir, 'source', 'discussion_forum', '*.txt'))
    elif dataset == 'r2v2':
    ...
    with open(output_file, 'w', encoding='utf-8') as w:
        for source_file in source_files:
            ...
            if dataset == 'normal':
                annotation_file = os.path.join(input_dir, 'rich_ere', 'discussion_forum',
...
def main():
    ...
    if model_name.startswith('bert-'):
        tokenizer = BertTokenizer.from_pretrained(model_name,
                                                  cache_dir=cache_dir)
    elif model_name.startswith('plm/bert-'):
        tokenizer = BertTokenizer.from_pretrained(model_name,
                                                  cache_dir=cache_dir)
    ...
    if args.split:
        all_data = []
        for ds in ["normal"]:
            with open(os.path.join(args.output,'{}.{}.oneie.json'.format("english",ds)), 'r', encoding='utf-8') as r:
        ...
        with open(os.path.join(args.output, 'train.json'), 'w') as w_train:
            json.dump(train_data, w_train, indent=4, ensure_ascii=False)
        with open(os.path.join(args.output, 'valid.json'), 'w') as w_dev:
            json.dump(valid_data, w_dev, indent=4, ensure_ascii=False)
        with open(os.path.join(args.output, 'test.json'), 'w') as w_test:
            json.dump(test_data, w_test, indent=4, ensure_ascii=False)
